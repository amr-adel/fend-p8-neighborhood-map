...
class App extends Component {
  ...
  componentDidMount() {
    fetch(
      `https://api.foursquare.com/v2/venues/search?near=cairo&limit=50&categoryId=4bf58dd8d48988d1fd941735&client_id=${process.env.REACT_APP_FOURSQUARE_CLIENT_ID}&client_secret=${process.env.REACT_APP_FOURSQUARE_CLIENT_SECRET}&v=20191020`
    )
      .then(res => res.json())
      .then(data => {
        this.setState({
  ...
  filterList = query => {
    this.setState({
      filteredList: this.state.malls.filter(
        mall =>
          mall.name.toLowerCase().indexOf(query.trim().toLowerCase()) !== -1
      ),
      selected: null
    });
  };
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <AsyncHOC
          asyncScriptOnLoad={this.AsyncHOCCallback}
          setSelected={this.setSelected}
          selectedId={selectedId}
          filteredList={filteredList}
          map={map}
